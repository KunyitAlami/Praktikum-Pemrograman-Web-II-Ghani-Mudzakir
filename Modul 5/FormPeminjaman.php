    ...
    <body>
        ...
        <div class="mt-24 mx-auto w-2/4 h-auto">
            ...
            <form class="max-w-sm mx-auto" method="post">
                    ...
                    <div class="mb-5">
                        <label for="id_member" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Siapa yang mau meminjam:</label>
                        <select name="id_member" id="id_member" required
                            ...
                            <?php
                            include "Koneksi.php";
                            $sql = mysqli_query($koneksi, "SELECT id_member, nama_member FROM member");
                            while ($data = mysqli_fetch_assoc($sql)) {
                                echo "<option value='" . $data['id_member'] . "'>" . $data['id_member'] . " - " . $data['nama_member'] . "</option>";
                            }
                            ?>
                        </select>
                    </div>
                    <div class="mb-5">
                        <label for="id_buku" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Buku apa yang mau dipinjam:</label>
                        <select name="id_buku" id="id_buku" required
                            ...
                            <?php
                            include "Koneksi.php";
                            $sql = mysqli_query($koneksi, "SELECT id_buku, judul_buku FROM buku");
                            while ($data = mysqli_fetch_assoc($sql)) {
                                echo "<option value='" . $data['id_buku'] . "'>" . $data['id_buku'] . " - " . $data['judul_buku'] . "</option>";
                            }
                            ?>
                        </select>
                    </div>
                    ...
                    <div class="mb-5">
                        <button name="kirim" class="text-white bg-gray-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Pinjam Buku</button>
                    </div>
            </form>
        </div>
    </body>
...
if (isset($_POST['kirim'])) {
    ...
    $hasil = tambahDataPeminjaman($koneksi, $tgl_pinjam, $tgl_kembali, $id_member, $id_buku);
}
?>